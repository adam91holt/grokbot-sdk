...
- For a calendar routine, the live host path is a dated cron (`43 18 18 8 *` — Pacific/Auckland local, annual repeat). `{ type: "once", at }` is not a host trigger; `createAgentAutomation` translates a standalone once to those host-local cron fields and appends a prompt footer telling the agent to delete the routine after it fires. Do not put once in a listener group (the host would drop it). Unrelated to `runOnce`.
