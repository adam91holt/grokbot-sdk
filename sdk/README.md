...
Routine automations on the live host are `{ type: "cron", schedule }` or event listeners (`slack`, `github`, …). A dated cron (`43 18 18 8 *`) is the host path that actually fires for a calendar time; it annual-repeats and there is no disable-after-fire contract. `{ type: "once", at }` is an SDK helper only (`onceToDatedCron` / create-update translation to that dated cron). Prefer ISO-8601 with a time and timezone; epoch ms must be `> 1e12`. Date-only and whitespace `at` are rejected. Do not put `once` in a group — the host would drop it and keep slack/etc. Unrelated to `runOnce` / `discussOnce`. The host must add `once` to its trigger allowlist before a true one-shot will exist.
