...
Routine automations use `{ type: "cron", schedule }` or `{ type: "once", at }` (`at` is ISO-8601 or epoch ms). `once` is a single fire — do not fake that with a dated cron (the UI would show an annual repeat). Unrelated to `runOnce` / `discussOnce`. The SDK parses and passes `once` through `createAgentAutomation` / `updateAgentAutomation` / disk `automation.json`; the live host scheduler must accept the shape before it will actually fire.
